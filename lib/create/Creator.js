...
module.exports = class Creator {
  ...
  async create(cliOptions = {}) {
    ...
    if (process.env.NODE_ENV === 'development') {
      directoryInfo = context.endsWith(_testOutputDir) ? _testOutputDir : _testOutputDir + `/${name}`
    } else {
      directoryInfo = name
    }
    ...
    console.log(
      ...
      (this.context === process.cwd() ? `` : chalk.cyan(` ${chalk.gray('$')} cd ${directoryInfo}\n`)) +
      chalk.cyan(`${shouldInstallDepend ? '' : ` ${chalk.gray('$')} ${hasYarn() ? 'yarn' : 'npm'} install\n`}`) +
      chalk.cyan(` ${chalk.gray('$')} ${hasYarn() ? 'yarn' : 'npm'} run serve`)
    )
    console.log()
