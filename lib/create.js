...
const templateContents = require('./create/templateContents')
...
async function create(projectName, options) {
  ...
  // See if is create in current directory
  const inCurrent = projectName === '.'

  // If create in current directory, get the folder name.
  let name
  const testOutputPath = path.resolve(cwd, _testOutputDir)
  if (process.env.NODE_ENV === 'development') {
    name = inCurrent ? 'pillar-cli' : projectName
  } else {
  ...
  if (process.env.NODE_ENV === 'development') {
    if (!fs.existsSync(testOutputPath)) {
      ...
      console.log(`\nCleaning ${chalk.cyan(testOutputPath)}...`)
      await fs.emptyDir(testOutputPath)
    }
    targetDir = inCurrent ? testOutputPath : path.resolve(testOutputPath, projectName)
  } else {
